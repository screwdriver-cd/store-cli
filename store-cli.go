...
var CacheCompressFormat string = strings.ToLower(os.Getenv("SD_CACHE_COMPRESS_FORMAT"))
...
func get(storeType, scope, key string) error {
	...
	if strings.ToLower(storeType) == "cache" && CacheStrategy == "disk" {
		return sdstore.Cache2Disk("get", scope, key, CacheCompressFormat, CacheCompress, CacheMd5Check, CacheMaxSizeInMB)
	} else {
...
func set(storeType, scope, filePath string) error {
	...
	if strings.ToLower(storeType) == "cache" && CacheStrategy == "disk" {
		return sdstore.Cache2Disk("set", scope, filePath, CacheCompressFormat, CacheCompress, CacheMd5Check, CacheMaxSizeInMB)
	} else {
		sdToken := os.Getenv("SD_TOKEN")
...
func remove(storeType, scope, key string) error {
	...
	if strings.ToLower(storeType) == "cache" && CacheStrategy == "disk" {
		return sdstore.Cache2Disk("remove", scope, key, CacheCompressFormat, CacheCompress, CacheMd5Check, CacheMaxSizeInMB)
	} else {
		sdToken := os.Getenv("SD_TOKEN")
		store := sdstore.NewStore(sdToken)
